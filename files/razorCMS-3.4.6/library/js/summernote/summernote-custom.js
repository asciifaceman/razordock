...
(function (factory) {
  ...
  if (typeof define === 'function' && define.amd) {
    ...
    define(['jquery', 'codemirror', 'cookie-monster'], factory);
  } else {
    // Browser globals: jQuery, CodeMirror
    factory(window.jQuery, window.CodeMirror);
  }
}(function ($, CodeMirror, monster) {
  ...
  var Dialog = function () {
    ...
    this.showImageDialog = function ($editable, $dialog) {
      return $.Deferred(function (deferred) {
        ...
        // fetch images here for browser
        $("#image-browser").html('<li style="text-align: center;"><i class="fa fa-circle-o-notch fa-spin" style="font-size: 60px;"></i></li>');
        $.ajax({
            url: RAZOR_BASE_URL + "rars/file/image",
            type: "GET",
            beforeSend: function(xhr) {xhr.setRequestHeader("Authorization", monster.get("token"));},
            success: function(data) 
            { 
              $("#image-browser").html('');
              for (var i = 0; i < data.imageList.length; i++) 
              {
                $("#image-browser").append(
                  '<li class="browse-image-select" style="width: 100px; height: 125px; cursor: pointer; overflow: hidden; display: inline-block; margin: 2px;">' +
                  '<div style="text-align: center; position: relative;">' +
                  '<i class="fa fa-times browse-image-delete" data-name="' + data.imageList[i].name + '" style="font-size: 20px; position: absolute; top: 2px; right: 2px; color: red;"></i>' +
                  '<div style="height: 80px;">' +
                  '<img src="' + data.imageList[i].url + '" style="max-width: 100%; max-height: 80px;"/>' +
                  '</div>' +
                  '<span style="display: inline-block; word-break: break-all; font-size: 10px">' + data.imageList[i].name + '</span>' +
                  '</div>' +
                  '</li>'
                );
              }

              $(".browse-image-select").click(function()
              {
                deferred.resolve($(this).find("img").attr("src"));
                $imageDialog.modal('hide');
              });

              $(".browse-image-delete").click(function(event)
              {
                event.preventDefault();
                ...
                $.ajax({
                  url: RAZOR_BASE_URL + "rars/file/image/" + $(this).attr("data-name"),
                  type: "DELETE",
                  beforeSend: function(xhr) {xhr.setRequestHeader("Authorization", monster.get("token"));},
                  success: function(data) 
                  { 
                    $(ele).closest(".browse-image-select").remove();
                  }
                });
              });
            }
    ...
    var tplDialogs = function (lang, options) {
      var tplImageDialog = function () {
        var body = '<h5>' + lang.image.selectFromFiles + '</h5>' +
                   ...
                   '</span>' +
                   '</div>' +
                   '<h5>' + lang.image.browseImages + '</h5>' +
                   '<ul id="image-browser" style="padding: 0px; list-style-type: none;">' +
                   '</ul>' +
                   '';
        return tplDialog('note-image-dialog', lang.image.insert, body);
      };
